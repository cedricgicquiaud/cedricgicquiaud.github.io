...
## Livraison 15 — Le build refuse un fichier absent ou lourd

Les tests ci-dessous se jouent sur `content/fiches/slice.md` (copie locale, à remettre en état ensuite par `npm run sync`) et sur des fichiers déposés dans `public/projets/slice/`. Aucun écran n'est concerné : cette livraison ajoute un contrôle au début de `npm run build`. Un fichier de test se fabrique sans image réelle : `head -c 307201 /dev/zero > public/projets/slice/gros.png` donne un fichier de 300 Ko + 1 octet ; `cp public/opengraph-image.png public/projets/slice/visuel.png` donne un vrai PNG (de 1200×630, donc refusé comme visuel, accepté comme capture de galerie).

### PFO-60 — Existence et poids des fichiers

- [ ] Sans rien changer, `npm run build` passe : la console commence par « check-assets : fichiers déclarés dans … conformes », puis les 7 lignes « écrit public/projets/generated/… », puis `next build`, puis « check-output : … propre ».
- [ ] Refus : ajouter `visuel: /projets/slice/absent.png` au frontmatter de `content/fiches/slice.md`, lancer `npm run build` : la commande échoue (code 1) dès la première étape avec « slice.md : visuel « /projets/slice/absent.png » : absent de public/ » ; aucune ligne « écrit public/projets/generated/… » ni `next build` ne s'affiche après.
- [ ] Refus : remplacer par un bloc `captures:` de deux entrées (`fichier: /projets/slice/a.png`, `legende: A` ; `fichier: /projets/slice/b.png`, `legende: B`) sans déposer les fichiers : deux lignes « slice.md : captures, entrée 1 « /projets/slice/a.png » : absent de public/ » et « … entrée 2 … ». Même chose avec `video:` (`fichier: /projets/slice/demo.mp4`, `duree: 2 min`) : « slice.md : video « /projets/slice/demo.mp4 » : absent de public/ ».
- [ ] Refus : déclarer `captures:` avec `fichier: /projets/slice/gros.png` et déposer le fichier de 300 Ko + 1 octet (commande ci-dessus) : « slice.md : captures, entrée 1 « /projets/slice/gros.png » : 307201 octets, plafond 300 Ko (307200 octets) ». Avec `head -c 307200 /dev/zero` (300 Ko juste), le poids passe et le message devient « ni PNG ni WebP d'après son contenu » (fichier d'octets nuls, tâche suivante).
- [ ] Refus : déclarer `video:` avec `fichier: /projets/slice/demo.mp4`, déposer `head -c 15728641 /dev/zero > public/projets/slice/demo.mp4` (15 Mo + 1 octet) : « slice.md : video « /projets/slice/demo.mp4 » : 15728641 octets, plafond 15 Mo (15728640 octets) ».

### PFO-61 — Type par signature et dimensions

- [ ] Déclarer `captures:` avec `fichier: /projets/slice/capture.png` et déposer `cp public/opengraph-image.png public/projets/slice/capture.png` (un vrai PNG de 1200×630) : `npm run build` passe, la capture de galerie n'a pas de dimensions imposées. Même résultat avec un vrai fichier `.webp` déposé à la place (n'importe quelles dimensions).
- [ ] Refus : renommer un vrai JPEG en `public/projets/slice/photo.png` (par exemple `sips -s format jpeg public/opengraph-image.png --out /tmp/photo.jpg && cp /tmp/photo.jpg public/projets/slice/photo.png`) et le déclarer en capture : « slice.md : captures, entrée 1 « /projets/slice/photo.png » : ni PNG ni WebP d'après son contenu ». L'extension `.png` ne suffit pas.
- [ ] Refus : déclarer `video:` avec `fichier: /projets/slice/demo.mp4` et y déposer une copie d'un PNG (`cp public/opengraph-image.png public/projets/slice/demo.mp4`) : « slice.md : video « /projets/slice/demo.mp4 » : pas un MP4 d'après son contenu (« ftyp » absent) ». Un vrai MP4 de moins de 15 Mo au même chemin passe.
- [ ] Refus : déclarer `visuel: /projets/slice/visuel.png` et y déposer `cp public/opengraph-image.png public/projets/slice/visuel.png` (1200×630) : « slice.md : visuel « /projets/slice/visuel.png » : 1200×630, attendu 1200×750 ». Un PNG de 1200×750 (`cp public/projets/generated/slice.png public/projets/slice/visuel.png` après un premier build) passe, et la console de `project-visuals` affiche « slice : visuel fourni /projets/slice/visuel.png, rien à générer ».

### PFO-62 — Build enchaîné et plafond de out/

- [ ] `grep '"build"' package.json` affiche exactement `node scripts/check-assets.mjs && node scripts/project-visuals.mjs && next build && node scripts/check-output.mjs` ; `npm run build` sur le dépôt tel quel passe dans cet ordre.
- [ ] Refus : avec `visuel: /projets/slice/absent.png` dans `content/fiches/slice.md`, `npm run build` échoue avant `next build` (aucune ligne « Generating static pages ») ; `git status public/` ne montre rien : le visuel généré n'a pas remplacé l'absent.
- [ ] Refus : `node -e "import('./scripts/check-output.mjs').then(m => console.log(m.checkOutput('out', [], 1024)))"` après un build affiche une liste d'un problème « poids total : N octets, plafond 1 Ko (1024 octets) » avec N le poids réel de `out/` (`du -sk out` pour comparer, à 1 Ko près). Sans troisième argument, la liste est vide : `out/` tient sous 150 Mo.
